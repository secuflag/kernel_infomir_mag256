...
static int uniperif_player_init_in_standby(struct uniperif_player *player)
{
	int lr_pol, result;
	...
	set__AUD_UNIPERIF_I2S_FMT__NUM_CH(player, 1);
	...
	/* Set clock divisor */
	set__AUD_UNIPERIF_CTRL__DIVIDER(player, DEFAULT_OVERSAMPLING / 64);
...
static int uniperif_player_deinit_standby(struct uniperif_player *player)
{
	unsigned long irqflags;
		...
		struct snd_pcm_runtime *runtime)
{
	...
	player->oversampling = player->info->oversampling;
...
static int uniperif_player_start(struct snd_pcm_substream *substream)
{
	...
	if (result) {
		dev_err(player->dev, "Failed to set clock rate; disable it");
		snd_stm_clk_disable(player->clock);
		return result;
	}
...
static int uniperif_player_register(struct snd_device *snd_device)
{
	...
	if (player->info->always_on)
		result = uniperif_player_init_in_standby(player);
		...
		struct uniperif_player *player)
{
	...
	if (of_property_read_u32(pnode, "mclk-fs",
				&info->oversampling))
		info->oversampling = DEFAULT_OVERSAMPLING;

	if (!of_property_read_u32(pnode, "always-on", &info->always_on) &&
	    info->always_on)
		info->suspend_delay = UNIPERIF_PLAYER_NO_SUSPEND;
...
static int uniperif_player_remove(struct platform_device *pdev)
{
	...
	if (player->info->always_on)
		uniperif_player_deinit_standby(player);
