...
{
	l2c_write_sec(val, l2x0_base, L2X0_DEBUG_CTRL);
}
#else
...
static void l2x0_lock_all(u32 cache_id)
{
	int lockregs;
	int i;
	u32 way_mask = 0xffff;
...
static void l2x0_disable(void)
{
	...
	l2x0_spin_lock_irqsave(flags);
	__l2x0_flush_all();
	if (!(readl_relaxed(l2x0_base + L2X0_AUX_CTRL)
			& BIT(L2X0_AUX_CTRL_NS_LOCKDOWN_SHIFT))) {
		l2c_write_sec(0, l2x0_base, L2X0_CTRL);
	} else {
		l2x0_lock_all(cache_id);
		__l2x0_inv_all();
	}
	dsb(st);
	l2x0_spin_unlock_irqrestore(flags);
...
static void l2x0_enable(u32 aux)
{
	u32 cache_id;
	...
	if (!(readl_relaxed(l2x0_base + L2X0_CTRL) & L2X0_CTRL_EN)) {
		...
		l2c_write_sec(aux, l2x0_base, L2X0_AUX_CTRL);

		l2x0_inv_all();

		l2c_write_sec(L2X0_CTRL_EN, l2x0_base, L2X0_CTRL);
	} else if (readl_relaxed(l2x0_base + L2X0_AUX_CTRL)
			& BIT(L2X0_AUX_CTRL_NS_LOCKDOWN_SHIFT)) {
		/*
		 * When L2 already enabled, and running in NS,
		 * ensure the L2 is unlocked.
		 */
		l2x0_unlock(cache_id);
	}
}
...
void __init l2x0_init(void __iomem *base, u32 aux_val, u32 aux_mask)
{
	...
	l2c_write_sec(0x3, l2x0_base, L2X0_POWER_CTRL);
				  ...
				  u32 *aux_val, u32 *aux_mask)
{
	...
	if (filter[1]) {
		...
		l2c_write_sec((filter[0] & ~(SZ_1M - 1)) | L2X0_ADDR_FILTER_EN,
...
{
	l2x0_enable(l2x0_saved_regs.aux_ctrl);
}
...
static void pl310_resume(void)
{
	...
	if (!(readl_relaxed(l2x0_base + L2X0_CTRL) & L2X0_CTRL_EN)) {
		...
		l2c_write_sec(l2x0_saved_regs.data_latency,
			l2x0_base, L2X0_DATA_LATENCY_CTRL);
		l2c_write_sec(l2x0_saved_regs.filter_end,
			l2x0_base, L2X0_ADDR_FILTER_END);
		l2c_write_sec(l2x0_saved_regs.filter_start,
			l2x0_base, L2X0_ADDR_FILTER_START);
		...
		if (l2x0_revision >= L2X0_CACHE_ID_RTL_R2P0) {
			l2c_write_sec(l2x0_saved_regs.prefetch_ctrl,
				l2x0_base, L2X0_PREFETCH_CTRL);
			if (l2x0_revision >= L2X0_CACHE_ID_RTL_R3P0)
				l2c_write_sec(l2x0_saved_regs.pwr_ctrl,
...
static void aurora_resume(void)
{
	if (!(readl(l2x0_base + L2X0_CTRL) & L2X0_CTRL_EN)) {
		l2c_write_sec(l2x0_saved_regs.aux_ctrl, l2x0_base,
				L2X0_AUX_CTRL);
		l2c_write_sec(l2x0_saved_regs.ctrl, l2x0_base, L2X0_CTRL);
	}
}
