...
static void st_ddr_pctl_shutdown(struct platform_device *pdev)
{
	int ret;

	ret = st_ddr_pctl_suspend(&pdev->dev);
	if (!ret)
		return;

	dev_err(&pdev->dev, "DDR shutdown failed -> Cold boot the system\n");
	BUG();
}
