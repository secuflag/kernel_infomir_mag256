...
/* Bidirectional pattern can be either 0x1 or 0x3 */
#define ST_GPIO_DIRECTION_BIDIR_1	0x1
#define ST_GPIO_DIRECTION_BIDIR_2	0x3

/* Output pattern is 0x2 */
#define ST_GPIO_DIRECTION_OUT		0x2

/* Input pattern can be either 0x0 or 0x4 */
#define ST_GPIO_DIRECTION_IN_1		0x0
#define ST_GPIO_DIRECTION_IN_2		0x4
...
static unsigned int st_gpio_direction_regs(struct st_gpio_bank *bank,
					   unsigned int pin_offset)
{
	...
	for (i = 0; i <= 2; i++) {
		...
		direction |= ((value >> pin_offset) & 0x1) << i;
	}
...
static int st_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	direction = st_gpio_direction_regs(bank, offset);

	return ((direction == ST_GPIO_DIRECTION_IN_1) ||
		(direction == ST_GPIO_DIRECTION_IN_2));
}
			...
			bool input)
{
	...
	st_gpio_direction(bank, gpio, input ?
		ST_GPIO_DIRECTION_IN_2 : ST_GPIO_DIRECTION_OUT);
		...
		struct pinctrl_dev *pctldev)
{
	...
	for (i = 0; i < ST_GPIO_PINS_PER_BANK; i++) {
		...
		if (save->function) {
			...
			save->direction =
				st_gpio_direction_regs(bank,
						       st_gpio_pin(pin_id));
			if ((save->direction != ST_GPIO_DIRECTION_IN_1) &&
			    (save->direction != ST_GPIO_DIRECTION_IN_2))
				st_gpio_get_output_value(bank, pin_id,
							 &save->out_val);
		}
	}
}
