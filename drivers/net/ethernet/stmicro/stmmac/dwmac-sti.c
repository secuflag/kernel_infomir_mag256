...
/* STi8416 register definitions */

/**
 *-------------------------------
 * src	 |BIT(6)| BIT(7)| BIT(9)
 *-------------------------------
 * MII   |  n/a	|   0	|   0
 *------------------------------
 * RMII  |  1	|   1	|   1
...
#define STI8416_RETIME_SRC_MASK		GENMASK(9, 7)
#define STI8416_ETH_PHYCLK_NOT_RXCLK		BIT(9)
#define STI8416_ETH_PHYCLK_NOT_TXCLK		BIT(8)
#define STI8416_ETH_INTERNAL_NOTEXT_PHYCLK	BIT(7)
...
static void sti8416_fix_retime_src(void *priv, u32 spd)
{
	struct sti_dwmac *dwmac = priv;
	u32 reg = dwmac->clk_sel_reg;
	u32 freq = 0;
	u32 val = 0;	/* Suitable for MII mode with txclk */

	if (dwmac->interface == PHY_INTERFACE_MODE_RMII) {
		val = STI8416_ETH_PHYCLK_NOT_RXCLK
		      | STI8416_ETH_PHYCLK_NOT_TXCLK;
		if (!dwmac->ext_phyclk) {
			val = STI8416_ETH_INTERNAL_NOTEXT_PHYCLK;
			freq = DWMAC_50MHZ;
		}
	...
	regmap_update_bits(dwmac->regmap, reg, STI8416_RETIME_SRC_MASK, val);
}

static int sti8416_init(struct platform_device *pdev, void *priv)
{
	struct sti_dwmac *dwmac = priv;
	u32 spd = dwmac->speed;
	...
	sti8416_fix_retime_src(priv, spd);
...
static void *sti8416_dwmac_setup(struct platform_device *pdev)
{
	struct sti_dwmac *dwmac	= sti_dwmac_setup(pdev);

	if (dwmac->clk_sel_reg < 0)
		return NULL;
...
const struct stmmac_of_data sti8416_dwmac_data = {
	.fix_mac_speed = sti8416_fix_retime_src,
	.setup = sti8416_dwmac_setup,
	.init = sti8416_init,
	.exit = sti_dwmac_exit,
};
